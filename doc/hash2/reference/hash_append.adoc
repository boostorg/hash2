...
struct hash_append_provider
{
    ...
    template<class Hash, class Flavor, class It>
    static constexpr void hash_append_range_and_size( Hash& h, Flavor const& f, It first, It last );

    template<class Hash, class Flavor, class It>
    static constexpr void hash_append_unordered_range( Hash& h, Flavor const& f, It first, It last );
};
```
